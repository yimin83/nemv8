      ...
      <template>
        ...
        <tbody>
          <tr v-for="zone in mergezones">
            <td class="text-center ma-0 pa-0 pt-3 pb-2 font-weight-bold black--text right-bold-border " style="cursor: pointer;background-color:rgb(294, 294, 189)" @mousedown.left='selectGraph(zone.zone.nZoneIdx)'>{{zone.ahu.ahu_name}}({{zone.ahu.ahu_desc}})</td>
    ...
    <v-dialog v-model='ahuGraphModal' persistent  height="950px"  width="1480px">
      <v-card height="900px">
        <v-toolbar dark icons-and-text>
          ...
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <v-btn icon dark @click="closeAhuGraph()">
        ...
        <v-card-text>
          <div>
            <v-row>
              <v-col class="d-flex pa-0 ma-0 mt-3 ml-3" cols="12" sm="8">
                &nbsp;&nbsp;
                <v-autocomplete
                  v-model="time.value"
                  :items="this.times"
...
export default {
  ...
  methods: {
    ...
    selectGraph (ahuNo){
      this.curAhuNo = ahuNo
      if (this.beFirst) {
        this.openAhuGraph(ahuNo)
      } else {
        this.searchGraph()
      }
      this.beFirst = false
    },
    closeAhuGraph: function () {
    ...
    getAhuTrend (ahuNo) {
      ...
      var startDateTime = this.startDate + ' ' + this.startTime
      var endDateTime = this.endDate + ' ' + this.endTime
      axios.put(`${this.$apiRootPath}rooms/solAhuTrend`, { ahuNo: ahuNo, startTime: startDateTime, endTime: endDateTime, time: this.time.value })
        .then((r) => {
          // this.datas = []
    ...
    searchGraph: function () {
      ...
      this.loading = true
      for (var i = 0; i < this.mergezones.length; i++) {
        if (this.mergezones[i].zone.nZoneIdx === this.curAhuNo) {
          this.graphTitle = this.mergezones[i].ahu.ahu_desc + '(' + this.mergezones[i].ahu.ahu_name + ')'
          break
        }
      }
    ...
    getAhuTrendSearch () {
      ...
      var startDateTime = this.startDate + ' ' + this.startTime
      var endDateTime = this.endDate + ' ' + this.endTime
      axios.put(`${this.$apiRootPath}rooms/solAhuTrend`, { ahuNo: this.curAhuNo, startTime: startDateTime, endTime: endDateTime, time: this.time.value })
        .then((r) => {
          // this.datas = []
          // this.datas.push('ahu': r.data, 'env':this.envDatas)
          this.datas = r.data
